...
export default function DropDown({title, icon:Icon}: DropDownProps){
    ...
    return(
        <div onClick={()=>isHidden(!hidden)} className='w-full flex items-center flex-col'>
        <div className='flex w-10/12 text-[#858585] justify-between items-center'>
        <div className='flex gap-4 items-center'><Icon className='w-[20px] h-[25px]'/> <span className='text-[17px]'>{title}</span></div>
